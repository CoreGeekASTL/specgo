...
本文指导如何将 specgo 插件安装到 OpenCode 或 Claude Code。specgo 是面向存量代码仓的四分类资产治理 skill 体系（7 个独立 skill：spec-analyze 资产分析（承载 17 个子流程，含资产质量审核）、spec-requirement-audit 需求审核（看文档+看代码）、spec-story-design story 设计、spec-code-generate 代码生成与测试、spec-pipeline 流水线执行、spec-update 资产刷新、mermaid-validate 图校验 + 17 个斜杠命令），详细功能见 [README.md](./README.md)。
...
1. **skill 已发现**：OpenCode 查看可用 skill 列表；Claude Code 用 `/plugin` 查看已装插件。应出现 **7 个独立 skill**：`spec-analyze`、`spec-requirement-audit`、`spec-story-design`、`spec-code-generate`、`spec-pipeline`、`spec-update`、`mermaid-validate`（17 个子流程在 spec-analyze 的 `references/subflows/` 下，不出现在 skill 列表属正常）
