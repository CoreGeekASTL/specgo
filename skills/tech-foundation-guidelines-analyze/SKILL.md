...
产出粒度对齐存量代码资产治理规范 v1.1——**仓级单篇**：

| 模式 | 产出 | 落盘 |
| --- | --- | --- |
| 起草模式 | 仓级单篇 `foundation-guidelines.md` | 被分析仓 `docs/tech/foundation-guidelines/` |
...
仓内无既有基础规范、用户也未提供规范文档时走本模式。盘点仓内基础编码机制使用现状——日志（级别使用 / 敏感信息脱敏 / 审计日志）、配置（配置读取方式 / 默认值处理 / 环境变量）、告警（告警 ID 使用 / 上报与恢复配对）——每机制一节：使用模式、调用点分布（文件路径）、应有约定建议，产出仓级单篇 `foundation-guidelines.md`，作为基础规范的事实基线（活文档，同名覆盖更新）。

### 差距分析模式

仓内已存在基础规范文档（`docs/tech/foundation-guidelines/foundation-guidelines.md`），或用户显式提供规范文件时走本模式。以规范为基准，对照扫描实际代码：逐机制、逐规范条目核查是否遵守，产出差距报告 `docs/tech/foundation-guidelines/report/{YYYYMMDD}-foundation-guidelines.md`（逐机制一节：合规项 / 差距项 / 规范未覆盖 / 规范条目无实现，各项附证据文件路径）。差距报告为次抛件，带日期、不覆盖。

guidelines 形态语义：基础规范是**指导性规范**（"应该"遵守），违反出报告提示改进，不做 CI 拦截——拦截是 standards 形态资产（如 qual-code-standards-analyze）的语义。

### 模式缺省回退

用户要求差距分析、但未提供规范文档且 `docs/tech/foundation-guidelines/` 下也无既有规范时，默认回退起草模式，并在产出文档末尾注明「规范未建，本次为现状提取 + 约定建议」。
...
- 否则检查被分析仓 `docs/tech/foundation-guidelines/foundation-guidelines.md` 是否存在 → 存在且用户意图是"检查 / 对照 / 差距"时按差距分析模式执行，规范来源记为该文档。
- 都没有 → 起草模式；若用户本意是差距分析，在文档末尾注明「规范未建，本次为现状提取 + 约定建议」。
...
仓内另有横切编码机制且具规范价值的（如监控埋点 metrics、链路追踪 traceId 传递、国际化文案），按实际探测结果增补机制小节；探测不到某类核心机制时，文档中该节标注「未发现」，证明排查过。
...
2. **调用点分布**：按机制的关注维度统计调用点并列出文件路径（不带行号），按模块聚类；调用点多时按代表性抽样精读，禁止全量阅读：
...
按 references/foundation-guidelines-template.md 填充，产出被分析仓 `docs/tech/foundation-guidelines/foundation-guidelines.md`：

- 元信息表 + 机制全景表（机制 / 使用框架或 SDK / 调用点分布概要 / 现状要点）；
- 每机制一节，固定三小节：**使用模式**（用法骨架 + 真实代码片段注明来源文件路径）、**调用点分布**（按关注维度列表，文件路径不带行号）、**应有约定建议**（基于现状归纳的"应该"级约定条目，逐条编号——编号供差距分析模式逐条对照引用，每条标注「现状已遵守 / 现状部分遵守 / 现状缺失」）；
- 约定建议从代码事实与业界通行做法归纳，不得脱离仓内现状凭空立法；
- 探测不到的机制整节标注「未发现」；业务语义推断不出标「待确认」；
- 同名文件已存在**直接覆盖更新**——规范文档是活文档，固定名、覆盖更新，git diff 即演进史。

### 第 5 步（差距分析模式）：对照规范核查并生成差距报告

以第 1 步确定的规范文档为基准，逐机制、逐规范条目核查第 3 步采集的事实：
...
- 产出差距报告 `docs/tech/foundation-guidelines/report/{YYYYMMDD}-foundation-guidelines.md`，按 references/gap-report-template.md 填充：结论概览表 + 逐机制一节（合规项 / 差距项 / 规范未覆盖 / 规范条目无实现，各项均附证据文件路径）。
...
- 基础规范文档（起草模式）：references/foundation-guidelines-template.md
...
- **实例 slug 从代码标识符派生**：本资产为仓级单篇、文件名固定 `foundation-guidelines.md`；文档内增补机制小节的 slug 取该机制的代码标识符（日志框架名 / 配置中心 client 名 / 告警 SDK 名）转 kebab-case，禁止 AI 自由起名。
- **活文档覆盖更新**：`docs/tech/foundation-guidelines/foundation-guidelines.md` 同名直接覆盖，不保留历史副本、不加日期后缀；**差距报告才带日期**，落 `report/` 子目录、次抛。
...
| 文件 | 用途 |
| --- | --- |
| references/foundation-guidelines-template.md | 起草模式基础规范文档模板（机制全景 + 每机制：使用模式 / 调用点分布 / 应有约定建议） |
| references/gap-report-template.md | 差距分析模式差距报告模板（结论概览 + 逐机制合规项 / 差距项 / 规范未覆盖 / 规范条目无实现） |
