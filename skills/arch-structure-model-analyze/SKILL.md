...
| 输入 | 粒度 | 画什么 | 产出 |
| --- | --- | --- | --- |
| 代码仓根路径 | 仓库级（模式 A） | 业务代码**第一层目录**之间的关系 | `docs/arch/structure-model/structure-model.md`（仓级总览 1 篇）+ `docs/arch/structure-model/structure-model-{module}.md`（每模块 1 篇） |
...
### 第 6 步：写仓级总览 structure-model.md
...
- 文件名：
  - 模式 A：总览 `structure-model.md`；每模块 `structure-model-{module}.md`，`{module}` 取模块目录名转 kebab-case。
...
- **不维护索引**：本 skill 不产出 README/索引文件——域索引 `docs/arch/README.md` 与总索引 `docs/README.md` 由 all-index skill 统一生成。
