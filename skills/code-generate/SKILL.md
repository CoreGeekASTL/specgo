...
代码生成收口后，产出**需求 → 代码全链路分析报告**，让人一眼看出改了什么、引用的资产靠不靠谱。本步无确认门，直接交付。
...
2. **派报告子代理**撰写报告（prompt 含上述取证材料 + 模板路径），按 references/code-generate-report-template.md 填充，固定两节：
...
4. 评估为「部分符合/不符合」的资产，主代理在回复中显式列出并给出下一步建议（如重跑某 analyze skill、补齐某资产后再生成）。
...
| 第 6 步 | 无确认门——分析报告为收尾交付物，直接落盘 docs/code-generate-report/ 并在回复中给出引用资产质量评估结论 |
