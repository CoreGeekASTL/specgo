...
<!-- 子流程：不独立暴露为 skill；由 specgo 主 skill 路由加载，也可由 /spec-function-design-audit 命令触发。依赖文件在同级 ../assets/（带短名前缀） -->
...
1. 通读文档全文。
2. 按 [../assets/function-design-audit--color-modeling.md](../assets/function-design-audit--color-modeling.md) 提取四类元素：粉「事件」、黄「角色」、绿「实体」、蓝「描述」，按时序/因果连接成链路。不臆造文档中不存在的规则——不确定的标记「待确认」。

## 阶段 2：全面扫描，生成初版 HTML

1. **表述质量扫描（必做）**：对照 [../assets/function-design-audit--requirement-quality-checklist.md](../assets/function-design-audit--requirement-quality-checklist.md) 逐句扫描 5 类——成分残缺/弱表述/术语失范/语义多解/带病放行。弱词黑名单：内置词表 ∪ 仓内 `docs/audit/weak-words.txt`（存在则追加）。
2. **逻辑断裂点扫描（必做）**：对照 [../assets/function-design-audit--gap-checklist.md](../assets/function-design-audit--gap-checklist.md) 断点 1-9。
3. **设计要素校验（含实现设计章节的文档必做，纯需求文档跳过）**：对照 gap-checklist.md 断点 10/11/12（时序图/验收用例/接口信息）。
4. 术语失范与断点 9 重叠的命中合并为一个断点。
5. 复制 [../assets/function-design-audit--model-template.html](../assets/function-design-audit--model-template.html)，填入建模结果与全部断点卡片（红色虚线高亮），保存到**源文档所在目录**，文件名 `{功能名}-建模结果.html`。
...
**必须询问用户**是否按 [../assets/function-design-audit--functional-design-template.md](../assets/function-design-audit--functional-design-template.md) 输出功能设计 md。确认后：
...
- [../assets/function-design-audit--color-modeling.md](../assets/function-design-audit--color-modeling.md) — 多彩建模方法论（阶段 1）
- [../assets/function-design-audit--requirement-quality-checklist.md](../assets/function-design-audit--requirement-quality-checklist.md) — 需求表述质量清单 5 类 + 弱词黑名单（阶段 2）
- [../assets/function-design-audit--gap-checklist.md](../assets/function-design-audit--gap-checklist.md) — 逻辑断裂点清单 12 类 + 严重程度分级 + ask-human 规范（阶段 2/3）
- [../assets/function-design-audit--model-template.html](../assets/function-design-audit--model-template.html) — 建模 HTML 模板（阶段 2/4）
- [../assets/function-design-audit--functional-design-template.md](../assets/function-design-audit--functional-design-template.md) — 功能设计模板（阶段 6）
