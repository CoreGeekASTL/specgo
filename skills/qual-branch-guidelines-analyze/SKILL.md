...
- 分支与变更规范文档（起草模式）：references/branch-guidelines-template.md
...
| 文件 | 用途 |
| --- | --- |
| references/branch-guidelines-template.md | 起草模式分支与变更规范文档模板（元信息 + 采样口径 + 四维度分节：现状分布表 + 约定建议） |
| references/gap-report-template.md | 差距分析模式差距报告模板（结论概览 + 逐维度合规项/差距项/规范未覆盖/规范条目无检查对象） |
