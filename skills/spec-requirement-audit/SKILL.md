...
description: >-
  需求审核 skill——基于多彩建模方法论审核 Spec/需求/功能设计文档：既看文档也看代码（代码对照：文档描述的既有行为/接口/规则/数据结构与代码事实比对，初步定位注入点/复用点），三类断点扫描（表述质量：句子成分残缺/弱表述/术语失范/语义多解 + 业务逻辑断裂点 + 设计要素：时序图/验收用例/接口信息）合并分级后统一澄清（默认 ask-human 批量提问、每问附基于代码事实的推荐做法；用户声明"以报告形式呈现"则落盘待澄清清单文档——源文档同目录 `{功能名}-待澄清清单.md`，等用户审核文档并逐条作答），澄清闭环后一次成型产出建模 HTML 与规范功能实现设计 md（功能设计规范格式），收尾输出不落盘审核报告（断点逐条 + 代码对照结论 + 产出清单）交用户审视。当用户提到"spec-requirement-audit"、"spec-function-design-audit"、"需求审核"、"审核 spec 业务逻辑完备性"、"多彩建模"、"检查需求逻辑断点"、"需求表述质量检查"、"需求歧义扫描"、"需求与代码对照"、"建模结果生成 HTML 并找人确认"、"功能设计文档校验"时使用。
---
...
| 输入 | 干什么 | 产出 |
| --- | --- | --- |
| 一篇 Spec/需求/功能设计文档 | 多彩建模 + 代码对照 + 三类断点扫描 + 澄清闭环，补齐到可供 story 设计 | 源文档同目录：`{功能名}-建模结果.html` + `{功能名}-规范功能实现设计.md`（报告模式另产 `{功能名}-待澄清清单.md`，闭环后保留留痕）；收尾：不落盘审核报告 |

**自包含原则**：规则与模板全部归档在自身 references/assets/ 下（mermaid 验证脚本除外，全插件共用）。

**交互双模式（全局条款）**：本 skill 所有询问点**默认使用 ask-human 工具**；若任务开始时用户声明"以报告形式呈现"（或同类意思），则全程**不使用 ask-human**——待澄清内容落盘为清单文档（源文档同目录 `{功能名}-待澄清清单.md`，按 clarification-template.md），等用户审核文档并逐条作答后继续；待审视内容以报告形式输出，等用户回复后继续。
...
6. **澄清（双模式）**：
   ...
   - **报告模式**（用户任务开始时声明"以报告形式呈现"）：不使用 ask-human——将全部待澄清内容按 [references/assets/clarification-template.md](references/assets/clarification-template.md) 落盘为 `{功能名}-待澄清清单.md`（**源文档同目录**，含编号/原文引用/类别/候选选项/推荐做法及理由/用户裁定填写位），对话内告知文件路径与状态统计，等用户审核文档并逐条作答。
7. **核对回答，循环追问**：已裁定的记录「人工裁定+答复时间」（报告模式同步回填清单文件：已裁定条目移入「已裁定」节归档）；未答/含糊的进下一轮；裁定引入新疑点的重新扫描生成新断点；循环至无新问题。报告模式每轮**同名覆盖更新**清单文件（轮次号+1）；澄清闭环后文件保留落盘，作为裁定留痕（阶段 5 报告断点清单可回链）。
...
- [ ] 澄清符合双模式：默认 ask-human 批量提问（一次抛出、每问 2-4 候选、推荐做法附代码依据、语义多解列全部理解）；报告模式落盘 `{功能名}-待澄清清单.md`（源文档同目录、按 clarification-template.md、逐轮同名覆盖）等用户审核作答，闭环后保留作裁定留痕；循环追问至无新问题
...
- [references/assets/clarification-template.md](references/assets/clarification-template.md) — 待澄清清单模板（阶段 3 报告模式落盘用）
