...
| 输入 | 干什么 | 产出 |
| --- | --- | --- |
| 一篇 Spec/需求/功能设计文档 | 多彩建模 + 代码对照 + 三类断点扫描 + 澄清闭环，补齐到可供 story 设计 | 源文档同目录：`{功能名}-建模结果.html` + `{功能名}-规范功能实现设计.md` + `{功能名}-待澄清清单.md`（澄清先行落盘，闭环后保留留痕）；收尾：不落盘审核报告 |

**自包含原则**：规则与模板全部归档在自身 references/assets/ 下（mermaid 验证脚本除外，全插件共用）。

**交互双模式（全局条款）**：待澄清内容**一律先落盘**清单文档（源文档同目录 `{功能名}-待澄清清单.md`，按 clarification-template.md），再分模式交互——**默认（ask-human 模式）**：落盘后用 ask-human 批量提问（用户也可直接在文件里作答）；**报告模式**（用户任务开始时声明"以报告形式呈现"或同类意思）：落盘后**不再询问**，等用户审核裁定文档内容、发新消息作答后继续。报告模式下其它待审视内容同样只以报告形式输出，全程不使用 ask-human。
...
6. **澄清（文件先行 + 双模式）**：先将全部待澄清内容按 [references/assets/clarification-template.md](references/assets/clarification-template.md) 落盘为 `{功能名}-待澄清清单.md`（**源文档同目录**，含编号/原文引用/类别/候选选项/推荐做法及理由/用户裁定填写位），再分模式交互：
   - **默认（ask-human 模式）**：落盘后一次抛出全部问题。每问含编号、原文引用、问题类别、2-4 个具体候选选项（**推荐项必须基于阶段 2 代码事实给出并标注理由**，不提供"其他"兜底）；语义多解类把 N 种理解全部列成选项。用户也可直接在清单文件里作答，以文件内容为准。
   - **报告模式**（用户任务开始时声明"以报告形式呈现"）：落盘后**不再询问**——对话内仅告知文件路径与状态统计，等用户审核裁定文档内容、发新消息作答。
7. **核对回答，循环追问**：已裁定的记录「人工裁定+答复时间」，**直接回填该问题卡的「用户裁定」栏与标题状态**（不另设汇总表——清单文件闭环后即完整裁定留痕）；未答/含糊的进下一轮；裁定引入新疑点的重新扫描生成新断点；循环至无新问题。每轮**同名覆盖更新**清单文件（轮次号+1）；澄清闭环后文件保留落盘（阶段 5 报告断点清单可回链）。
...
- [ ] 澄清符合"文件先行 + 双模式"：全部待澄清内容先落盘 `{功能名}-待澄清清单.md`（源文档同目录、按 clarification-template.md、逐轮同名覆盖、裁定直接回填问题卡）；默认模式落盘后 ask-human 批量提问（一次抛出、每问 2-4 候选、推荐做法附代码依据、语义多解列全部理解）；报告模式落盘后不再询问、等用户发新消息作答；循环追问至无新问题；闭环后文件保留作裁定留痕
...
- [references/assets/clarification-template.md](references/assets/clarification-template.md) — 待澄清清单模板（阶段 3 澄清先行落盘用）
