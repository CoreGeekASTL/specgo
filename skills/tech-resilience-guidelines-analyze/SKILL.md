...
产出文档中含 ```mermaid 代码块时（本资产通常为纯表格与代码片段文档，不含图），交付前必须运行 mermaid-validate skill 的本地验证脚本逐文件校验：
...
- 每维度韧性规范文档：references/dimension-template.md
...
| 文件 | 用途 |
| --- | --- |
| references/readme-template.md | 主文档模板（元信息 + 五维度导航表） |
| references/dimension-template.md | 每维度文档模板（使用说明 + 代码案例） |
