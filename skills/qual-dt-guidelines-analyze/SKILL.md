...
产出文档中含 ```mermaid 代码块（测试金字塔图等）时，交付前必须运行 spec-mermaid-diagram skill 的本地验证脚本逐文件校验：
...
- DT 规范文档（起草模式）：references/dt-guidelines-template.md
...
| 文件 | 用途 |
| --- | --- |
| references/dt-guidelines-template.md | 起草模式 DT 规范文档模板（元信息 + 测试现状盘点 + 金字塔基线 + 用例设计方法 + 自测报告要求 + 覆盖率门禁红线 + 附注） |
| references/gap-report-template.md | 差距分析模式差距报告模板（结论概览 + 覆盖率红线违反（CI 门禁）+ 合规项/差距项/规范未覆盖/规范条目无实现） |
