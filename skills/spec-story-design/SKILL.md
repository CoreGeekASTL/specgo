...
3. **目标功能文档存在性判定（决定第 5 步是修改还是新增）**：检查 `docs/story/` 下是否已有**同功能域**文档。判定依据按优先级：需求文档所述功能名与既有文档功能名一致 → 业务语义相同（解决同一业务问题）→ 核心模块高度重叠。判定结果二选一：
   - **已有同功能域文档** → 本需求是对该功能的增强/迭代，第 5 步走「修改既有文档」分支
   - **无同功能域文档** → 第 5 步走「新增文档」分支
   - 归属拿不准时：列出判定依据（功能名/语义/模块重叠度）与用户确认，不擅自决定——改错文档比多问一句代价大
4. **存量代码（按需抽查）**：仅核实复用点/注入点的真实文件路径与分层约定（models/dao/service/controllers/routers），禁止全仓阅读。
...
- 在既有 `feature-<功能名>.md` 上**按节更新**，禁止整篇重写覆盖：
  ...
  - 关键数据结构 / 框架引用 / AI 编码指南：增量补充，与既有内容去重
