...
3. **落地地址**（src/controllers/auth_controller.go）：
...
- [ ] 修改方案均为四段式：变更本质（纯业务语言、无代码符号）→ 行为对照表（场景穷举、不变标注"不变"）→ 落地地址（修改/新增 xx 函数 ≤27 字）→ 边界与约束
