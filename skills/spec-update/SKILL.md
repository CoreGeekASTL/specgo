...
3. **mermaid 验证**：`node <specgo插件目录>/skills/spec-analyze/scripts/validate-mermaid.mjs <刷新文档路径>`，全部 VALID 才算完成；INVALID 按报错修复后重验。
...
- **mermaid 验证**：含 mermaid 的刷新文档按 `<specgo插件目录>/skills/spec-analyze/references/mermaid-guide.md` 的语法红线绘制，并过 `skills/spec-analyze/scripts/validate-mermaid.mjs` 全部 VALID。
