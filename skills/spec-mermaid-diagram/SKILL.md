...
- 任何资产整理/设计类 skill（spec-structure-analyze、spec-feature-analyze、spec-story-design、spec-interface-analyze、spec-external-call-analyze、spec-framework-usage-analyze、spec-logic-audit、spec-asset-refresh 等）执行到收尾阶段，产出物中含 mermaid 图。
...
两档方法，按优先级选用。**方法 A 为默认**：确定性、离线、可重复，直接调用 mermaid 官方解析器。
...
- 每张图一行 `[VALID] 文件 第N个图(起始行X) (图类型)` 或 `[INVALID] ...` + 解析错误定位。
- 全部 VALID 退出码 0；任一 INVALID 退出码 1。**退出码非 0 必须修复后重验**。

### 方法 B：mmdc 渲染级验证（需 headless Chrome）

能发现"语法合法但渲染崩"的问题，但安装重：

```bash
npm install -g @mermaid-js/mermaid-cli
npx puppeteer browsers install chrome-headless-shell
mmdc -i diagram.mmd -o /tmp/test.png   # 成功即合法
```

注意：`mmdc` 报 `Could not find Chrome` 是环境问题，图本身没问题，改用方法 A 验证，不要去改正确的图。

## 工作流程

1. **选类型**：按"图类型速查"表确定图类型。
2. **编写**：写 `.mmd` 或直接在 md 里写 ```mermaid 块，遵守"语法红线"，label 默认加引号。
3. **验证**：用方法 A 跑验证脚本，必须全部 VALID。
4. **修复循环**：INVALID 时按错误行号定位修复，重验，直至全部 VALID。
5. **交付**：报告验证结果（几张图、全部 VALID）。
...
- **验证前置**：任何含 mermaid 图的产出物，未通过方法 A 验证不得宣称完成。
- **修复基于报错**：INVALID 时按解析器给出的行号与期望 token 修复，禁止整图重写碰运气。
- **不误改好图**：渲染器环境问题（如 Chrome 缺失）不得作为改图理由，先换方法复核。
- **脚本依赖一次性安装**：`scripts/` 下的 `node_modules` 不入库（已 gitignore），换机器后重跑 `npm install` 即可。
