...
{一句话，≤60 字、3 行以内：聚合是什么、解决什么业务问题。一致性边界、对象构成、模型层定位依据等详情不写入本节，见下方类图、对象说明表与补充说明}
...
{2~4 句：聚合不变量与一致性边界说明（如"OrderItem 只能随 Order 整体读写"）；跨聚合引用方式（持标识还是持对象）；领域服务的编排范围；模型层定位依据（对象如何被识别为模型层）}
