...
<!-- 子流程：不独立暴露为 skill；由 spec-analyze 主 skill 路由加载，也可由 /biz-data-model-analyze 命令触发。依赖文件在同级 ../assets/（带短名前缀） -->
...
按 ../assets/data-model--data-model-template.md 的骨架产出（概述 + ER 图 + 字段表 + 数据生命周期 + 缓存数据结构 + 补充说明）：
...
严格按 ../assets/data-model--data-model-template.md 骨架填充——只填占位符、表格行、图；写作规则见上文各步骤与「关键约束」。
...
- ../assets/data-model--data-model-template.md — 数据模型文档模板（概述 + ER 图 + 字段表 + 数据生命周期 + 缓存数据结构 + 补充说明，第 4 步用）
