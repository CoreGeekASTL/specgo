...
<!-- 子流程：不独立暴露为 skill；由 spec-analyze 主 skill 路由加载，也可由 /tech-basic-mechanism-guidelines-analyze 命令触发。依赖文件在同级 ../assets/（带短名前缀） -->
...
- **主文档 `README.md`** 按 ../assets/basic-mechanism-guidelines--readme-template.md 填充：元信息表 + 机制全景表（维度 / 使用框架或 SDK / 函数清单概要 / 子文档链接）+ 附注（排查到但无法判定归属 / 待人工确认的机制或调用点）。
- **每维度子文档** 按 ../assets/basic-mechanism-guidelines--dimension-template.md 填充，固定两小节：**函数调用说明**（函数清单表格：函数 / 作用 / 参数说明 / 定义文件）、**使用代码案例**（真实代码片段，每段注明来源文件路径）。
...
产出文档中含 ```mermaid 代码块（如机制调用关系图）时，交付前必须运行 mermaid 本地验证脚本逐文件校验：
...
- 主文档 README 索引：../assets/basic-mechanism-guidelines--readme-template.md
- 每维度基础规范文档：../assets/basic-mechanism-guidelines--dimension-template.md
...
| 文件 | 用途 |
| --- | --- |
| ../assets/basic-mechanism-guidelines--readme-template.md | 主文档模板（元信息 + 机制全景表 + 附注） |
| ../assets/basic-mechanism-guidelines--dimension-template.md | 每维度文档模板（函数调用说明 + 使用代码案例） |
