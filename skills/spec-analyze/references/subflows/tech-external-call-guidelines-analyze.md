...
<!-- 子流程：不独立暴露为 skill；由 spec-analyze 主 skill 路由加载，也可由 /tech-external-call-guidelines-analyze 命令触发。依赖文件在同级 ../assets/（带短名前缀） -->
...
主文档按 ../assets/external-call-guidelines--readme-template.md 填充（元信息表 + 外部服务全景表）；子文档按 ../assets/external-call-guidelines--service-template.md 填充（按协议分二级小节，每个外部调用接口一个三级章节：业务场景、接口功能、调用位置、协议信息——协议/封装方式/超时重试/错误码处理现状）。
...
- 产出差距报告 `docs/0-tech/external-call-guidelines/report/{YYYYMMDD}-external-call-guidelines.md`，按 ../assets/external-call-guidelines--gap-report-template.md 填充：结论概览表 + 逐服务一节（合规项 / 差距项 / 规范未覆盖，各项均附证据文件路径）。
- 差距报告**只新增不覆盖**，文件名带日期；同日重跑同名覆盖。

### 第 6 步：验证 mermaid 图可渲染（收尾必做）

产出文档中含 ```mermaid 代码块（下游依赖全景图等）时，交付前必须运行 mermaid 本地验证脚本逐文件校验：
...
- README 索引（提取模式）：../assets/external-call-guidelines--readme-template.md
- 每服务通信规范文档（提取模式）：../assets/external-call-guidelines--service-template.md
- 差距报告（差距分析模式）：../assets/external-call-guidelines--gap-report-template.md
...
- **索引分工**：域索引 `docs/0-tech/README.md` 与总索引 `docs/README.md` 自 v3.0 起不再自动生成（spec-index 已移除），本 skill 不维护。
- **与相邻资产互补**：入站接口看 biz-interface-analyze 产出的 `docs/0-biz/interface/`；纯故障策略（熔断 / 降级）的专项规范归 tech-resilience-guidelines-analyze，本 skill 只记录调用点的超时/重试/错误码处理事实与合规性。

## 参考文件索引

| 文件 | 用途 |
| --- | --- |
| ../assets/external-call-guidelines--readme-template.md | 提取模式 README 索引模板（元信息 + 外部服务全景表 + 附注） |
| ../assets/external-call-guidelines--service-template.md | 提取模式每服务文档模板（接口清单 + 按协议分节，含协议信息四项） |
| ../assets/external-call-guidelines--gap-report-template.md | 差距分析模式差距报告模板（结论概览 + 逐服务合规项/差距项/规范未覆盖） |
