...
<!-- 子流程：不独立暴露为 skill；由 spec-analyze 主 skill 路由加载，也可由 /qual-code-standards-analyze 命令触发。依赖文件在同级 ../assets/（带短名前缀） -->
...
仓内无既有编码规范、用户也未提供规范文档时走本模式。以内置规则底稿 [../assets/code-standards--rules-catalog.md](../assets/code-standards--rules-catalog.md)（27 条通用 clean code 规则 + 安全附加项 S1~S3 + Go/Java/Python/C++ 语言特则，每条已预标注检查方式与默认级别）为基础，结合仓内代码现状（主语言、框架、存量风格）裁剪：选取仓内主语言对应的语言特则章节（其余语言章节剔除）、按仓内约定调整可调参数与默认级别、收录仓内特有约定为新增条目，产出仓级单篇 `docs/0-qual/code-standards/code-standards.md`。裁剪决策必须记录在该文档「七、裁剪说明」节，保证规范演进可追溯。
...
按 [../assets/code-standards--code-standards-template.md](../assets/code-standards--code-standards-template.md) 填充，产出 `docs/0-qual/code-standards/code-standards.md`：
...
5. 按 [../assets/code-standards--gap-report-template.md](../assets/code-standards--gap-report-template.md) 填充产出 `docs/0-qual/code-standards/report/{YYYYMMDD}-code-standards.md`：「一、红线违反」节标题与表格列名固定（CI 解析锚点，禁止改动），红线违反非空时元信息「门禁结论」填**拦截**。
...
全部 VALID 才算完成；INVALID 按报错行号定位修复后重验，禁止跳过。画图规则见 `<specgo插件目录>/skills/spec-analyze/references/mermaid-guide.md` 的「语法红线」。

## 输出模板

- 编码规范文档（起草模式）：[../assets/code-standards--code-standards-template.md](../assets/code-standards--code-standards-template.md)
- 差距报告（差距分析模式）：[../assets/code-standards--gap-report-template.md](../assets/code-standards--gap-report-template.md)
- 规则底稿（起草裁剪基础 / 差距分析代位依据）：[../assets/code-standards--rules-catalog.md](../assets/code-standards--rules-catalog.md)
...
| 文件 | 用途 |
| --- | --- |
| [../assets/code-standards--rules-catalog.md](../assets/code-standards--rules-catalog.md) | 内置规则底稿：通用 27 条 + 安全附加项 S1~S3 + 语言特则（Go G1~G10 / Java J1~J6 / Python P1~P5 / C/C++ C1~C4），每条预标注检查方式与默认级别，附惯例冲突豁免判定表（第 2、3、4 步用） |
| [../assets/code-standards--code-standards-template.md](../assets/code-standards--code-standards-template.md) | 起草模式 code-standards.md 模板（总览 + 红线规则 + 建议规则 + 安全编码红线 + 禁止项清单 + 豁免与裁量 + 裁剪说明）（第 3 步用） |
| [../assets/code-standards--gap-report-template.md](../assets/code-standards--gap-report-template.md) | 差距分析模式差距报告模板（红线违反节为 CI 解析锚点 + 建议级差距 + 合规项 + 豁免裁量 + 规范未覆盖 + 规范条目无实现）（第 4 步用） |
