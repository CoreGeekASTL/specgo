...
<!-- 子流程：不独立暴露为 skill；由 spec-admin 主 skill 路由加载，也可由 /spec-init 命令触发。依赖文件在同级 ../assets/（带短名前缀） -->
...
摘要末尾提示后续动作：运行各 analyze skill 填充资产、运行 spec-index 生成索引。

## 输出模板

本 skill 不产出资产内容文档，唯一产出是**迁移执行摘要**（直接呈现给用户，不落盘，用户要求时可落盘为 `docs/spec-init-迁移摘要-{YYYYMMDD}.md`）：
...
- **不维护索引**：域索引与总索引归 spec-index，本 skill 不创建、不更新任何 README 索引（迁移 `README.md` 文件本身除外）。
