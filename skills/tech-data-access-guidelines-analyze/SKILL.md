...
存储文档按分类选模板填充——内存数据套 references/memory-data-template.md，持久化数据套 references/persistent-data-template.md。两模板骨架一致，章节顺序即叙事主线：

1. **元信息表**：分支 / 更新日期 / Skill / 运行模式 / 数据分类（内存数据 / 持久化数据）。
2. **数据设计与定位**（现状 + 约定）：模板重点之一——存什么数据、为什么放这种形态、结构设计（key/字段/索引/容量/TTL）、数据生命周期。
3. **使用方式**（现状 + 约定）：模板重点之二——读写路径、一致性、并发控制（内存数据）或连接/事务/分页批量/SQL 防护（持久化数据）、降级行为。
4. **适用场景**：模板重点之三——场景表（业务场景 / 何时使用该数据 / 不适用时的兜底），从访问点所在函数/模块/注释推断，推断不出标「待确认」，禁止臆造。
5. **访问点分布表**：访问点 / 所在文件 / 所在函数 / 访问方式（封装层或裸 client，封装层附文件路径）/ 业务场景；同一访问方式多处调用逐行列出；**预留死代码单列**——封装存在但无任何业务调用方的，不计入分布表，在「附注」节逐条列出。
6. **错误处理**（现状 + 约定）：分类处理、重试、兜底。
7. **附注（可选）**：预留死代码、配置声明了但代码无实际访问的存储，逐条列出附文件路径。

各「约定」小节从现状主导用法归纳、对风险点（裸 SQL 拼接、无 TTL、无容量上限、无连接池上限等）给出应有规则，整节标注「起草待评审」；该存储不涉及的子维度写「本存储不涉及」，不删除章节标题。

规则：

- 业务场景不得臆造：从访问点所在函数/模块/注释推断，推断不出标「待确认」。
- 约定从代码事实归纳，不从外部文档照抄；现状内部不一致（同一存储多种并存用法）时如实并列描述，约定给出统一方向。
...
- **与相邻资产互补**：持久化存储的表结构/字段/生命周期细节看 biz-data-model-analyze 产出的 `docs/biz/data-model/`（本资产重"怎么用、什么场景用"，数据模型资产重"字段与关系"）；框架用法骨架（无规范、纯现状）看 tech-framework-usage-analyze 产出的 `docs/tech/usage/`；纯故障策略（熔断/降级/重试）的专项规范归 tech-resilience-guidelines-analyze，本 skill 只记录数据访问点的错误处理事实与合规性。
...
| 文件 | 用途 |
| --- | --- |
| references/memory-data-template.md | 起草模式内存数据文档模板（元信息 + 数据设计与定位 + 使用方式 + 适用场景 + 访问点分布 + 错误处理 + 附注） |
| references/persistent-data-template.md | 起草模式持久化数据文档模板（同骨架，使用方式节换为连接/事务/分页批量/SQL 防护子维度） |
