...
| 产出 | 内容 | 落盘 |
| --- | --- | --- |
| 域索引 | 本域各资产目录 + 目录内文件清单（每文件一句话说明） | 被分析仓 `docs/{域}/README.md`（每有资产的域 1 篇） |
...
本 skill 与被分析仓语言/框架无关：只读 `docs/`，不读代码。资产目录内的 README（如 `docs/biz/interface/README.md` 主文档）归各 analyze skill 维护，本 skill 只产出 `docs/{域}/README.md` 与 `docs/README.md`，不覆盖资产级 README。
...
| 资产目录 | 资产中文名 | 内涵一句话 |
| --- | --- | --- |
| `arch/structure-model` | 结构模型 | 模块划分、分层、职责与依赖关系（UML 包图 + 依赖矩阵） |
| `arch/interaction-model` | 交互模型 | 模块间主业务流程、消息走向（UML 时序图） |
| `biz/interface` | 接口 | 服务对外接口清单，按功能域聚类 |
| `biz/rules` | 业务规则 | 按需求类整理"条件 → 动作 + 依据"规则条目 |
...
| `tech/resilience-guidelines` | 韧性规范 | 超时/重试/熔断/异常处理 |
| `tech/foundation-guidelines` | 基础规范 | 日志/配置/告警等编码指导 |
| `qual/code-standards` | 编码规范 | 命名、注释、函数长度/圈复杂度、安全编码红线、禁止项清单 |
| `qual/dt-guidelines` | DT规范 | 测试金字塔与覆盖基线、用例设计方法、覆盖率门禁 |
| `qual/branch-guidelines` | 分支与变更规范 | 分支模型、commit/MR 规范、评审要求 |

### 第 2 步：逐域生成域索引 docs/{域}/README.md
...
2. **服务依赖全景图**（mermaid `flowchart LR`）：
   - 数据源：`docs/tech/comm-guidelines/comm-guidelines-{service}.md`，每篇文档 = 一个下游服务节点（`{service}` 取文件名去 `comm-guidelines-` 前缀与 `.md` 后缀）。
   ...
   - `docs/tech/comm-guidelines/` 缺失或其内无服务文档 → 本节**不画空图、不臆造依赖**，只写一行「通信规范资产未建（docs/tech/comm-guidelines/ 缺失），请先运行 tech-comm-guidelines-analyze 提取出站调用后再生成全景图」。
...
- 域索引 `docs/{域}/README.md`：references/domain-readme-template.md（域名 + 生成时间 + 各资产节：内涵一句话 + 文件清单表 + report/ 标注）
...
- **活文档覆盖更新**：`docs/{域}/README.md` 与 `docs/README.md` 同名直接覆盖，不保留历史副本、不加日期后缀（差距报告才带日期，索引不带）；每次生成标注生成时间；重跑幂等。
