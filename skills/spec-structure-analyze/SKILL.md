...
文档最终落到被分析代码仓根目录的 `docs/architecture/module-structure/` 下（不存在则创建）。
...
- 对每条依赖记录证据 `文件路径`（不记行号），供表格"主要依赖"列引用。
...
### 第 7 步：落盘 docs/architecture/module-structure/

- 文档保存到**被分析代码仓根目录**的 `docs/architecture/module-structure/` 下（不是当前工作目录的 `docs/architecture/module-structure/`，除非两者相同）。
- 模式 A 文件名：`spec-structure-{仓库名}.md`，仓库名取 git 仓库名或根目录名。
- 模式 B 文件名：`spec-structure-{目录名}.md`。
- `docs/architecture/module-structure/` 不存在则创建（连同 `docs/` 一起创建）。
- 若同名文件已存在，覆盖前提示用户确认。
...
成品文档严格按 references/structure-template.md 骨架填充，只填占位符、表格行、图；写作规则见上文各步骤与「关键约束」。
...
- **只读不改**：只读、只分析、只产出文档，不改动被分析代码仓的任何文件（新建 `docs/architecture/module-structure/` 目录和文档文件除外）。
