...
| `spec-code-generate` | 代码生成与本地自测试：零 TODO 完整实现；两线并行——测试子代理只写自测用例（从契约出发）、实现子代理只写实现（按文件分组可并行），主代理执行测试、失败归因并全权修正（单测/集成测试/验证命令实跑取证） | 完整可运行的代码 + 测试（落被分析仓） |
...
flowchart LR
    ...
    subgraph G2["需求开发阶段（每需求一轮）"]
        D1["spec-requirement-audit<br/>需求审核"] --> D2["spec-story-design<br/>story 设计"] --> D3["spec-code-generate<br/>编码与测试"] --> D4["spec-report<br/>总结报告"] --> D5["spec-update<br/>资产刷新"]
    end
    G1 -.-> G2
```
...
sequenceDiagram
    ...
    H-->>A: "拍板"
    A->>H: "总结报告：三节报告（准确性/资产使用/用户反馈）审视"
    H-->>A: "拍板（反馈写入报告）"
...
第 6 步：总结报告
输入指令：`生成总结报告`
（spec-report；三节：代码生成的准确性 / 资产使用情况 / 用户反馈，落盘 docs/1-storys/{功能名}/{YYYYMMDD}-report.md）
...
**第二步：需求开发（日常使用）**。把需求文档路径发给 AI，依次说"审核这份需求文档"→"做 story 设计"→"按 develop-task 实现代码"→"生成总结报告"→"刷新 docs 资产"，每个 skill 收尾的报告审视拍板即可。
